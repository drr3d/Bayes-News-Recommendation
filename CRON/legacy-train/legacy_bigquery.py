...
# ~~~~~~~~~~~~~
logger = logging.getLogger('memory_profile7_log')
logger.setLevel(logging.DEBUG)

from memory_profiler import profile
# create file handler which logs even debug messages
fh = logging.FileHandler("memory_profile7.log")
...
import sys
sys.stdout = LogFile('memory_profile7_log', reportIncrementFlag=False)
# ~~~~~~~~~~~~~
...
def main(df_input, df_current, current_date, G,
    ...
    #                             'is_general', 'rank']].loc[model_transform['user_id'].str.contains('1610c8e6b6466b-05a6ac1ba8c1b8-4323461-100200-1',
    #                                                                                                 case=False)].head(45)
    # print "len(model_transform): %d" % len(model_transform)
    ...
    if savetrain:
        ...
        if str(saveto).lower() == "datastore":
            ...
            if multproc:
                mh.saveDatastoreMP(model_transformsv)
                # mh.saveDataStorePutMulti(model_transform[['user_id', 'topic_id', 'is_general', 'rank']])
            else:
