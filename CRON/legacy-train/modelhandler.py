...
# ~ just change to your, but it doesnot work on windows env
# jsongcrendential_path = "C:\\Users\\Norman\\"
# jsongcredential_fname = "topic-recommender-staging.json"
# os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = jsongcrendential_path + jsongcredential_fname
...
def loadDSHistory(key_list, kinds='topic_recomendation_history'):
    ...
    # query.add_filter('topic_id', '=', topid)
    # return query.fetch()
    keys = list()
    for key in key_list:
        keys.append(client.key(kinds, key))

    return client.get_multi(keys=keys)
